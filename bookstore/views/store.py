...
@store.route('/custom',methods=['GET', 'POST'])
@login_required
def custom():
    if request.method == 'POST':
        # 在這裡處理表單的提交邏輯，例如儲存資料到資料庫等
        # 然後重定向到 custom_complete
        return redirect(url_for('bookstore.custom_complete'))
    
    # 如果是 GET 請求，渲染 custom.html
    return render_template('custom.html', user=current_user.name)

@store.route('/custom_complete',methods=['GET', 'POST'])
def custom_complete():
    return render_template('custom_complete.html')
